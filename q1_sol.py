...
def fix_broken_tycho(entries: pd.DataFrame) -> pd.DataFrame:
    ...
    fixed_entries: pd.DataFrame = (entries.drop(  columns=['country', 'url'])
                                          ...
                                          .assign(disease=lambda df: df.disease.str.replace(pat=alt_name_reg, repl='', regex=True))
                                          .assign(year   =lambda df: df.epi_week.apply(get_year) )
                                          .query('1890 <= year <= 1895')
...
if __name__ == "__main__":
    ...
    broken_entries: pd.DataFrame = pd.read_csv("data/tycho-broken-22.csv", sep=",")
